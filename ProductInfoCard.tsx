...
export default function DrinkCard({ product }: { product: Data }) {
  ...
  return (
    <div className="p-4 shadow-md hover:shadow-lg transition-shadow">
      ...
      <div className="flex items-start gap-4 items-center justify-center text-center mt-16">
        <div className="flex-shrink-0">
          <Image
            ...
            width={200}
            height={200}
            className="w-120 h-120 object-cover rounded-md"
          />
        </div>
        <div>
          <h3 className="font-bold text-2xl mb-2">{product.name}</h3>
          <p className="text-gray-100 mb-4 max-w-prose mb-4">{product.description}</p>
          <p className="text-base text-gray-200 mb-4">Category: {product.category.name}</p>
          <p className="text-base text-gray-200 mb-4">Price: ${product.price}</p>
        </div>
      </div>
    </div>
  );
